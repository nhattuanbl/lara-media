...
class WorkflowStubManager
{
    ...
    public static function make($class, ?string $connection = null, ?string $queue = null): static
    {
        $storedWorkflow = config('workflows.stored_workflow_model', StoredWorkflow::class)::create([
            'class' => $class,
            ...(isset($connection) ? ['connection' => $connection] : []),
            ...(isset($queue) ? ['queue' => $queue] : []),
        ]);
    ...
    private function dispatch(): void
    {
        ...
        /** @var Queueable $job */
        $job = $this->storedWorkflow->class::$dispatch(
            $this->storedWorkflow,
            ...Y::unserialize($this->storedWorkflow->arguments)
        );

        if ($this->storedWorkflow->conection) {
            $job->onConnection($this->storedWorkflow->connection);
        }

        if ($this->storedWorkflow->queue) {
            $job->onQueue($this->storedWorkflow->queue);
        }
    }

    public function workFlow(): StoredWorkflow
    {
        return $this->storedWorkflow;
    }
}
